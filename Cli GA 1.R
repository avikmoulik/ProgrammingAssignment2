...
sqrt(sum(((data$cli-data$vec)*(data$cli-data$vec)),na.rm = TRUE))
...
sqrt(sum(((data$cli-data$vec_fin)*(data$cli-data$vec_fin)),na.rm = TRUE))
...
for (ib in 1:no_gene){
  chk_vec_scaled <- sort(499+((gene_mat[,ib] - min(gene_mat[,ib]) )/(max(gene_mat[,ib]) - min(gene_mat[,ib]) ))*(14000-499))
  chk_vec_scaled_grp <- chk_vec_scaled[-length(chk_vec_scaled)]
  chk_cost_data <- as.numeric(as.character(cut(data$cli,breaks = chk_vec_scaled,labels = chk_vec_scaled_grp  )))
  chk_cost <- sqrt(sum((data$cli-chk_cost_data)*(data$cli-chk_cost_data)))
  best_counter[ib] <- chk_cost
}
best_ind <- which.min(best_counter)
...
mutated_vec <- gene_mat[,pos[1]]+f*(gene_mat[,pos[2]]-gene_mat[,pos[3]])+f*(gene_mat[,best_ind]-gene_mat[,i])
